...
import { addParticlesPanel, addLevaDebug } from '../Utils/Debug';

import BackgroundConfig from '../Config/particles2D.js';

export default function Background2D({particlesContainer}){
    ...
    let config = addLevaDebug("particles", {
        autoplay: {
    ...
    const particlesLoaded = useCallback(async function(container){
        await addParticlesPanel(container.particles._array);
    }, []);
    return <>
